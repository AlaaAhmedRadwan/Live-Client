package com.codesroots.live_venodr.helper

class Constants {
    companion object {
        const val BASE_URL = "https://satatechnologygroup.net:5102/api/"
        const val IMAGE_URL = "https://satatechnologygroup.net:5102/"
        const val WebView_URL = "http://satawebsite.codesroots.com/"
        const val SocketURL = "https://satatechnologygroup.net:5100"
        const val RANDOM_URL = "api/breeds/image/random"
        var GOOGLE_GEOCODE_URL = "https://maps.googleapis.com/maps/api/geocode/json"

    }
}